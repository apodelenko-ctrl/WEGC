...
export async function verifyAccess(request,env,fetcher=fetch,now=Date.now()) {
  ...
  try {
    ...
    if (body.iss!==issuer || !Array.isArray(body.aud) || !body.aud.includes(env.ACCESS_AUDIENCE) || !Number.isFinite(body.exp) || body.exp<=seconds || !Number.isFinite(body.iat) || body.iat>seconds+30 || (body.nbf!==undefined && (!Number.isFinite(body.nbf)||body.nbf>seconds+30))) throw new Error('invalid');
