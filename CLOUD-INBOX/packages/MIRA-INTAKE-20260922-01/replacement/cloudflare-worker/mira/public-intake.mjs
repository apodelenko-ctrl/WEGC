...
async function submit(request,env,fetcher){
 ...
 await env.MIRA_DB.batch([
  ...
  s(env,`INSERT INTO mira_intake_events(id,request_id,actor,status,version,created_at) SELECT id,id,'public_request',status,0,created_at FROM mira_intake_requests WHERE id=?`,id)
 ]);
...
/** Called only after existing Access verification and active membership lookup. */
export async function routeIntakeAdmin(request,env,m,identity){
 const url=new URL(request.url),root='/mira/api/admin/intake';
 ...
 if(request.method==='GET'&&url.pathname===root){
  ...
  const rows=(await s(env,'SELECT id,company,city,name,email,email_verified,assigned_subject,status,version,created_at,updated_at,public_response FROM mira_intake_requests WHERE id>? ORDER BY id LIMIT ?',cursor,limit+1).all()).results;
  return json({records:rows.slice(0,limit),next_cursor:rows.length>limit?rows[limit-1].id:null,scope:'Unverified connection requests only. No email sends or membership activation.'});
 }
 const match=url.pathname.match(/^\/mira\/api\/admin\/intake\/([0-9a-f-]+)\/review$/i);check(match&&request.method==='POST',404,'not_found');
 const id=uuid(match[1]),b=await body(request);keys(b,['version','status','response']);
 check(Number.isSafeInteger(b.version)&&b.version>=0,400,'invalid_version');
 ...
 const row=await one(env,'SELECT * FROM mira_intake_requests WHERE id=?',id);check(row,404,'receipt_not_found');check(row.version===b.version,409,'stale_version');
 check(transitions[row.status]?.includes(b.status),409,'invalid_transition');
 // Explicit operator publication to receipt holder; never an email/Telegram send.
 const response=b.response===null?null:text(b.response,1,2000);
 check(!['responded','needs_information'].includes(b.status)||response,400,'public_response_required');
 const event=crypto.randomUUID(),now=new Date().toISOString();
 const results=await env.MIRA_DB.batch([
  s(env,'UPDATE mira_intake_requests SET status=?,public_response=?,version=version+1,updated_at=?,last_event_id=? WHERE id=? AND version=?',b.status,response,now,event,id,b.version),
  s(env,`INSERT INTO mira_intake_events(id,request_id,actor,status,version,public_response,created_at) SELECT ?,id,?,status,version,public_response,updated_at FROM mira_intake_requests WHERE id=? AND last_event_id=?`,event,identity.subject,id,event)
 ]);
 check(results[0].meta.changes===1,409,'stale_version');return json({id,status:b.status,version:b.version+1,published_to_receipt:true,external_send:false});
}
