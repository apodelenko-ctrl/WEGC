...
3. Import verified endpoint bindings through `src/admin.py --db PATH bind PRIVATE.json`. A binding contains `contact`, `context` (including `role: agency`, name, city, `crm_account_id`), `channel`, `account`, `peer`, and `evidence`. Use numeric provider IDs, not a claimed username/phone in message text. Conflict => review, not automatic merge. Unknown inbound endpoints go to `identity_review`; bind then `resolve EVENT_ID`. Never fuzzy-merge by company name.
4. Create private `MIRA_KB` JSON array of approved answer blocks: `{id,text,source,audience,approved,valid_until,contact_ids?}`. `valid_until` is an actual review expiry as Unix seconds, not an invented product validity. Use `contact_ids` for restricted terms. Do not bulk approve `wegc-kb.js`: historical prices/contract claims require current source checks. With no valid answer, the system creates a handoff task and pauses.
