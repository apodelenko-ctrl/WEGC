...
const batch=read('batch-005-togliatti-observations.json'), master=read('master-agencies.json'), contacts=read('contacts.json');
...
for(const row of batch.records){
 ...
 a.entity_status='public_business_identity_legal_registry_may_remain_pending';a.branch_status='local_business_candidate_no_federal_network_signal';
 a.next_action='Verify remaining legal independence, overseas activity and missing public work channels before outreach';
 ...
 for(const e of a.field_evidence.filter(e=>e.batch_id===batch.batch_id&&e.value&&/phone|email|telegram|whatsapp|vk_url|other_social_url/.test(e.field))){
  ...
  contacts.records.push({agency_id:a.agency_id,...e,channel_kind:e.field==='telegram_url'&&a.telegram_kind==='public_channel'?'public_channel':e.field.startsWith('direct_')?'public_person_work_contact':e.field==='other_social_url'?'public_company_social':'public_company_contact',direct_endpoint_published:e.field.startsWith('direct_'),direct_dialogue_verified:false});
 }
}
const reviewed=master.records.filter(a=>a.city==='Тольятти'&&a.field_evidence.some(e=>e.batch_id===batch.batch_id));
...
assert.equal(suitable,10);assert.equal(reviewed.length,10);assert.equal(added,expectedAdded);assert.equal(master.records.length,before+10);
...
const counters={checked_at:batch.checked_at,scope:'Canonical provisional candidates; channel counts are companies with fresh verified fields in Togliatti B005 reviewed set',canonical_candidate_groups:master.records.length,RU:country('Russia'),BY:country('Belarus'),other:master.records.length-country('Russia')-country('Belarus'),new_candidates_this_block:added,existing_records_enriched:0,reviewed_records:reviewed.length,first_wave_suitable_candidates:suitable,separate_overseas_or_network_segment:0,named_role:reviewed.filter(a=>has(a,'contact_first_name')&&has(a,'contact_last_name')&&has(a,'contact_role')).length,phone:count('company_phone'),general_email:count('general_email'),direct_business_phone:count('direct_business_phone'),direct_public_work_email:count('direct_public_work_email'),whatsapp_published_link:count('whatsapp_url'),telegram_direct:reviewed.filter(a=>has(a,'telegram_url')&&a.telegram_kind==='direct_dialogue').length,telegram_channel:reviewed.filter(a=>has(a,'telegram_url')&&a.telegram_kind==='public_channel').length,vk:count('vk_url'),other_social:count('other_social_url'),fully_enriched:reviewed.filter(a=>['company_phone','general_email','direct_business_phone','direct_public_work_email','whatsapp_url','telegram_url','vk_url','contact_first_name','contact_last_name','contact_role'].every(f=>has(a,f))).length,crm_imports_this_task:0,external_sends:0,cities_target_complete:3,togliatti_remaining_to_10:0,existing_rows_deleted:0,native_crm_agency_snapshot:78,native_crm_snapshot_source:'CLOUD-INBOX/receipts/MIRA-MKT-20260918-01.json',native_crm_not_added_to_research:true};
...
save('master-agencies.csv',csv(master.records,columns));save('contacts.json',contacts);save('COUNTS.json',counters);save('batch-005-togliatti.csv',csv(reviewed,columns));
const cityRows=fs.readFileSync(dir+'coverage-by-city.csv','utf8').split('\n'),header=cityRows[0].split(','),ci=cityRows.findIndex(l=>l.includes('RU-CITY-003'));assert(ci>0);
const coverage={city_id:'RU-CITY-003',city:'Тольятти',segment:'core_300k_to_1m',target_min_agencies:10,canonical_candidates:reviewed.length,verified_suitable:suitable,remaining_to_minimum:0,official_population_status:'blocked_official_source',research_status:'minimum_met_fields_incomplete'};
cityRows[ci]=header.map(k=>esc(coverage[k])).join(',');save('coverage-by-city.csv',cityRows.join('\n'));
const resume=read('RESUME.json');Object.assign(resume,{sourceCommitRead:batch.source_commit,canonicalCandidateGroups:master.records.length,canonicalRussiaGroups:country('Russia'),newAgenciesCollectedThisTask:(resume.newAgenciesCollectedThisTask||0)+added,freshFirstWaveSuitableCandidates:suitable,latestBatch:'batch-005-togliatti-observations.json',lastCheckpointAt:batch.checked_at,completedCityIds:[...new Set([...(resume.completedCityIds||[]),'RU-CITY-003'])],nextCityId:'RU-CITY-004',latestCheckpointEvent:'CLOUD-INBOX/events/2026-09-18/cloud-both-bases-b005.json',remaining:'Togliatti minimum met at 10/10 research-fit public business identities. Five records have a public phone and five a general email; direct person channels remain sparse. Official city-proper population is blocked.',nextStep:'continue_city_cursor_RU-CITY-004; official population still requires primary verification'});save('RESUME.json',resume);
save('POPULATION-CHECK-B005.json',{checked_at:batch.checked_at,...batch.population_check});save('rejected-or-held.json',{records:master.records.filter(a=>a.first_wave_fit!==true).map(a=>({agency_id:a.agency_id,segment:a.segment,first_wave_fit:a.first_wave_fit,reason:a.fit_reason})),source_rows_deleted:0});
save('B005-TEST-REPORT.json',{id_preserved:true,unique_ids:true,before_at_source_commit:before,after:master.records.length,added_this_checkpoint:added,replay_actual_additions:added,existing_enriched:0,togliatti_reviewed:reviewed.length,togliatti_suitable:suitable,separate_overseas_or_network:0,crm_imports:0,external_sends:0});console.log(JSON.stringify(counters,null,2));
