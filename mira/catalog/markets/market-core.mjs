...
export const MARKETS=Object.freeze({phuket:{name:'Пхукет',country:'Таиланд',path:'/mira/catalog/'},bali:{name:'Бали',country:'Индонезия',path:'/mira/catalog/bali/'},dubai:{name:'Дубай',country:'ОАЭ',path:'/mira/catalog/dubai/'}});
...
export function marketFromPath(path){const p=String(path).split('?')[0];for(const m of ['bali','dubai'])if(p.startsWith(`/mira/catalog/${m}/`)||p.startsWith(`/mira/catalog/projects/${m}-`))return m;return 'phuket';}
export function isCatalogueIndex(path){return /^\/mira\/catalog\/(?:bali\/|dubai\/)?(?:index\.html)?$/.test(path);}
...
export function combineProjects(base,extra){
 if(!Array.isArray(base)||base.length!==618)throw Error('Phuket source not reconciled');
 const all=[...base.map(p=>({...p,market:'phuket'})),...validateExpansion(extra).projects];
...
export function filterProjects(all,state={}){const words=normalize(state.q).split(/\s+/).filter(Boolean);return all.filter(p=>(!state.district||p.district===state.district)&&(!state.family||p.family===state.family)&&(!state.kind||(p.propertyTypes?p.propertyTypes.includes(state.kind):p.kind===state.kind))&&words.every(w=>normalize([p.name,p.district,p.family,p.summary].filter(Boolean).join(' ')).includes(w)));}
