...
class FullCatalogTests(unittest.TestCase):
 ...
 def test_unverified_prices_and_generic_images_excluded(self):
  d=m.load(ROOT);self.assertEqual(5,sum(bool(p['image']) for p in d['projects']));self.assertTrue(all('price' not in p for p in d['projects']))
 def test_all_detail_pages_exist_and_registration_is_closed(self):
  for p in m.load(ROOT)['projects']:
   text=m.detail(p);self.assertIn('Регистрация клиента закрыта',text);self.assertIn('disabled',text);self.assertNotIn('type="email"',text);self.assertIn(m.esc(p['name']),text)
